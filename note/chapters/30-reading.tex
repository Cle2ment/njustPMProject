...
\begin{tabular}{c|c|c|c|c}
\hline
\textbf{i} & \textbf{交换对} & \textbf{a[i]} $\leftrightarrow$ \textbf{a[n-1-i]} & \textbf{结果} \\
\hline
0 & a[0] $\leftrightarrow$ a[3] & 5 $\leftrightarrow$ 8 & [\textbf{8}, 6, 7, \textbf{5}] \\
1 & a[1] $\leftrightarrow$ a[2] & 6 $\leftrightarrow$ 7 & [8, \textbf{7}, \textbf{6}, 5] \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item 对称下标：a[i] $\leftrightarrow$ a[n-1-i]。
...
\hline
0 & 'a' (97) & 'a' (97) & a[i] $\geq$ b[i] \checkmark & a[i] & 97-32=65 & 'A' \\
1 & 'b' (98) & 'c' (99) & a[i] $\geq$ b[i] $\times$ & b[i] & 99-32=67 & 'C' \\
2 & 't' (116) & 'i' (105) & a[i] $\geq$ b[i] \checkmark & a[i] & 116-32=84 & 'T' \\
...
\noindent \textbf{第 1 层}：Fun(5, 4)，b=4 $\geq$ 1 → \texttt{p = Fun(4, 2) + 5*5 = Fun(4, 2) + 25}

\noindent \textbf{第 2 层}：Fun(4, 2)，b=2 $\geq$ 1 → \texttt{p = Fun(3, 0) + 4*4 = Fun(3, 0) + 16}
...
\textbf{延伸：} 可以让学生思考——如果把递归终止条件换成 b == 0，会有什么不同？（Fun(3,0)仍然触发，但 Fun(3,1) 也会继续递归直到 b$\leq$0。）
